...
// Replace manifest link (might not match exactly, so replacing generally or inserting)
html = html.replace(/<link rel="manifest"[^>]*>/, `<link rel="manifest" href='data:application/manifest+json;utf8,{"name":"مودیریەتی فێرخوازان","short_name":"فێرخوازان","start_url":".","display":"standalone","background_color":"#ffffff","theme_color":"#4f46e5","icons":[{"src":"data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>🎓</text></svg>","sizes":"512x512","type":"image/svg+xml"}]}'>`);
...
// Remove old SW script if exists
html = html.replace(/<script>\s*if\s*\('serviceWorker'[^<]*<\/script>/g, '');
